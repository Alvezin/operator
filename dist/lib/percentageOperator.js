...
var moneyOperator = __importStar(require("./MoneyOperator"));
...
function XIsHowPercentOf(fractionOfvalue, totalvalue, options) {
    var func = ((fractionOfvalue * 100) / totalvalue).toFixed(2);
    var withSymbol = "".concat(func, "%");
...
function getCompoundInterest(capital, interestRate, time, options) {
    var rate = (interestRate / 100);
    var result = capital * (operator.getPotentiation((1 + rate), time));
    if (options) {
